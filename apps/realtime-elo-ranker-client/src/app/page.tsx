...
export default function Home() {
  ...
  const updateLadderData = useCallback((player: PlayerData) => {
    setLadderData((prevData) => {
      const index = prevData.findIndex(p => p.id === player.id);
      let newData;
      if (index === -1) {
        // Si le joueur n'existe pas, on l'ajoute
        newData = [...prevData, player];
      } else {
        // Sinon, on met à jour le joueur existant
        newData = prevData.map(p => p.id === player.id ? player : p);
      }
      return quickSortPlayers(newData);
    });
  }, []);
